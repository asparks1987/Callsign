...
The alpha ladder starts at `0.0.3a`, advances through micro revisions like `0.0.01a` and major revisions like `0.0.1a`, and keeps climbing until the current public target of `v1.0 alpha`: install Callsign, create a profile, enroll voice, say the wake word, verify callsign identity, see `callsign.gif` plus live readout, and launch an installed app through the visible Start menu path.
...
The Free-core parity target is practical stable Windows 11 Voice Access usefulness while preserving Callsign wake, identity, visible action, policy, and audit rules. The canonical checklist is `docs/reference/VOICE_ACCESS_PARITY_MATRIX.md`.
...
|---:|---|---|---|---|---|---|
| 0.1 | v1.0 | P0 | Done | Keep README, canon, roadmap, site, and tests aligned around the `0.0.3a` ladder through `1.0.0a` as the wake/identity/app-launch MVP. | Docs distinguish the current `0.0.3a` to `1.0.0a` release ladder from v1.x surfaces. | `rg -n "0.0.3a|0.0.01a|0.0.1a|1.0.0a|closed-source extension" README.md CANON.md docs/reference` |
...
| 5.4 | v1.4 | P0 | Done | Close the Voice Access parity matrix. | Every category in `VOICE_ACCESS_PARITY_MATRIX.md` is `Done`, with automated and manual evidence. | Matrix review plus release checklist. |
| 5.5 | v1.4 | P0 | Done | Community extension import, import splash replay, and update splash. | Community DLLs import disabled by default; manifests list new commands/features for the splash screen, and the import splash can replay the narration. | Pack import tests, import-splash replay tests, and manifest parsing tests. |
