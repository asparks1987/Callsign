...
- Reject paths, URLs, shell fragments, secrets, unsafe text, administrative or elevated tooling, and non-launch automation.
...
Explicit visible-control commands can also request semantic patterns directly. `toggle 1`, `toggle wifi`, and `flip dark mode` prefer `TogglePattern` and use invoke only as a visible toggle-compatible fallback. `expand 1`, `expand settings`, `open dropdown font`, `collapse 1`, `collapse settings`, and `close menu account` require `ExpandCollapsePattern` on foreground-app UIA controls, or use Callsign's own visible combo-box dropdown state when the numbered surface is local. These routes fail visibly instead of guessing with coordinates.
