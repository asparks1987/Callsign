...
public static class CallsignVisualStyle
{
    ...
    public const string SurfacePrinciples = "compact, high-contrast, translucent, non-activating, accessible, high-contrast-aware, text-scaling-aware, reduced-motion-aware, visible-status";
    public const string EvidenceMarker = "contrast>=4.5:1; opacity=0.86-0.99; radius=20-26px; Segoe UI; high-contrast-ready; text-scale-ready; reduced-motion-safe; stop-visible";
    ...
    public const float MinimumTextScale = 1.0f;
    public const float MaximumTextScale = 1.6f;
    ...
    public readonly record struct VisualPalette(
        Color SurfaceBackground,
        Color ElevatedSurfaceBackground,
        Color PrimaryText,
        Color SecondaryText,
        Color Accent,
        Color OverlayText,
    ...
    public static VisualPalette DefaultPalette { get; } = new(
        SurfaceBackground,
    ...
    public static VisualPalette HighContrastPalette { get; } = new(
        SystemColors.Window,
        SystemColors.Window,
    ...
    public static VisualPalette GetPalette(bool highContrast) =>
        highContrast ? HighContrastPalette : DefaultPalette;

    public static float ClampTextScale(float requestedScale) =>
        float.IsNaN(requestedScale) || requestedScale < MinimumTextScale
            ? MinimumTextScale
            : Math.Min(requestedScale, MaximumTextScale);

    public static string DescribeAccessibilityMode(bool highContrast, float requestedTextScale, bool reducedMotion)
    {
        var contrastMode = highContrast ? "high-contrast palette" : "default palette";
        var motionMode = reducedMotion ? "reduced motion" : "standard motion";
        return $"{contrastMode}; text-scale={ClampTextScale(requestedTextScale):0.##}; {motionMode}; {EvidenceMarker}.";
    }
