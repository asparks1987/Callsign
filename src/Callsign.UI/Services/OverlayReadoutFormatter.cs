...
public static class OverlayReadoutFormatter
{
        ...
        bool dictationActive = false)
    {
        ...
        return state switch
        {
            ...
            AlphaSessionState.WaitingForCommand when !string.IsNullOrWhiteSpace(verifiedCallsign) => "Identity confirmed. Say the app name.",
            AlphaSessionState.WaitingForCommand => identityRetryPrompt ?? "Identity confirmed. Say the app name.",
